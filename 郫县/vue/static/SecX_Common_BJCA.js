...
/*
// if (!document.getElementById("XTXAPP")) {
try {
	...
	document.writeln("<OBJECT classid=\"CLSID:3F367B74-92D9-4C5E-AB93-234F8A91D5E6\" height=1 id=XTXAPP style=\"HEIGHT:1px;LEFT:1px;TOP:1px;WIDTH:1px\" width=1 VIEWASTEXT></OBJECT>");
	document.writeln("<OBJECT classid=\"CLSID:3BC3C868-95B5-47ED-8686-E0E3E94EF366\" height=1 id=picobj style=\"HEIGHT:1px;LEFT:1px;TOP:1px;WIDTH:1px\" width=1 VIEWASTEXT></OBJECT>");
	//creatDomObjectNode('CLSID:3F367B74-92D9-4C5E-AB93-234F8A91D5E6', 'XTXAPP', 'display:none', true);
	//creatDomObjectNode('CLSID:3BC3C868-95B5-47ED-8686-E0E3E94EF366', 'picobj', 'display:none', true);
	XTXAPP.SOF_GetVersion();
} catch (e) {
	console.log(e);
...
function GetUserList() {
	return "张三||102000004733814/6002201412016948&&&李四||102000004733815/6002201412016949&&&";
	var strUserList;
	try {
...
// 获取唯一标识号
function GetUniqueIDGXCA(cert) {
	debugger;
	var oid = "33"; //GXCA cert
	//var cert;
	//cert = SOF_ExportUserCert(key, KEY_SIGNATURE);
	var certdn = XTXAPP.SOF_GetCertInfo(cert, oid);
	if (certdn == "") {
		alert("读取用户证书主题失败");
		return false;
	}
	var sub = certdn.split(",");
	var sf_id = "";
	var m;
	for (m = 0; m < sub.length; m++)//通过“，”号找到ID号码
	{
		//内网证书写入的ID号
		var get_i = sub[m].indexOf("EmployeeID - ");
		if (get_i >= 0) {
			sf_id = sub[m].substring(get_i + 13);
			return sf_id;
		} else {
			//return "";
		}
...
function getUsrSignatureInfo(key) {
	...
	//HashData("test"); // 由于ca的问题，必须先hash，才能取到图片,2012年8月份以后新项目可以不加该语句。
...
function testCAOther() {
	// alert("hello ca");
	return "hello ca";
}

function creatDomObjectNode(classid, id, style, ifVIEWASTEXT) {
