      ...
      <table class="orderList__body orderList__table"
        ...
        <tbody>
          <order-item class="orderList__row"
                      ...
                      :style="{color:order.ordStatDesc=='停止'?'red':''}"
...
export default {
  ...
  watch: {
    data() {
      if (
        ...
        this.data.orders.sort(
          utils.compareByTwoProperty2("bedIndex", "createDateTime")
        );
        // }
      }
    },
    requestQueryFlag(value) {
      if (value) {
        ...
        let selectedPatientNum = 0;
        this.selectedPatients.forEach((episodeID) => {
          if (!isNaN(episodeID)) {
            selectedPatientNum += 1;
          }
        });
        if (
          selectedPatientNum > searchInfo.searchNum &&
          (searchInfo.excutedOrderFlag || searchInfo.printedOrderFlag)
        ) {
          this.$message.error(
            `已执行/已打印的查询人数上限为${searchInfo.searchNum}!`
          );
          this.finishQuery();
          return;
        }
  ...
  computed: {
    ...mapGetters(["requestQueryFlag", "searchInfo", "selectedPatients"]),
    getTableStyle() {
      let width = 0;
