...
import systemApi from '../store/api/systemConfig';
...
var utils = {
  ...
  // createPrintCommObject: function () {
  //   debugger;
  //   let webIP = session.USER.WEBIP;
  //   const printCommObject = document.createElement('OBJECT');
  //   printCommObject.id = 'DHCCNursePrintComm';
  //   printCommObject.setAttribute('classid', `${webIP}/dhcmg/DHCCNursePrintComm.dll#DHCCNursePrintComm.WebInterface`);
  //   printCommObject.setAttribute('iewastext', '');
  //   printCommObject.setAttribute('style', 'display:none;')
  //   document.body.appendChild(printCommObject);
  // },
  getPrintDll: function () {
    let printDLL = document.getElementById('DHCCNursePrintComm');
    // if (!printDLL) {
    //   utils.createPrintCommObject();
    //   printDLL = document.getElementById('DHCCNursePrintComm');
    // }
    return printDLL;
  },
  /**
  ...
  compareByTwoProperty2: function (propName1, propName2) {
    return function (obj1, obj2) {
      const val1 = String(obj1[propName1]);
      const val2 = String(obj2[propName1]);
      const val21 = obj1[propName2];
  ...
  */
   ...
   *            ]
   */
  splitChunk(chunks, size, ...argus) {
    if (argus[0].length !== 0) {
      chunks.push(
        argus.length > 1
          ? argus.map(array => array.splice(0, size))
